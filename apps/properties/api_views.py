...
from .serializers import (
    ...
    RoomWriteSerializer, RoomFacilitySerializer,
)
...
class PropertyListCreateAPI(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        qs = Property.objects.select_related('owner').prefetch_related('photos', 'rooms').all()
        ...
        if rent_type: qs = qs.filter(rent_type=rent_type)
        if prop_type: qs = qs.filter(type=prop_type)
...
class PropertyDetailAPI(generics.RetrieveUpdateDestroyAPIView):
    ...
    queryset           = Property.objects.select_related('owner').prefetch_related('photos', 'rooms__photos', 'rooms__facility')
...
class RoomListCreateAPI(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        ).select_related('property__owner').prefetch_related('photos', 'facility')
...
class RoomFacilityAPI(APIView):
    ...
    def get(self, request, room_id):
        ...
        if not facility:
            return Response(
                {'detail': 'No facilities added yet.'},
    ...
    def post(self, request, room_id):
        ...
        if facility:
            # Update existing
            serializer = RoomFacilitySerializer(facility, data=request.data, partial=True)
        else:
            # Create new
...
class RoomSearchAPI(generics.ListAPIView):
    ...
    def get_queryset(self):
        qs = Room.objects.select_related(
            'property__owner'
        ).prefetch_related('photos', 'facility').all()
        ...
        if city:       qs = qs.filter(property__city__icontains=city)
        if status:     qs = qs.filter(status=status)
        if room_type:  qs = qs.filter(room_type=room_type)
        if min_price:  qs = qs.filter(rent_price__gte=min_price)
        if max_price:  qs = qs.filter(rent_price__lte=max_price)
        if furnishing: qs = qs.filter(furnishing=furnishing)
        if wifi == 'true':
