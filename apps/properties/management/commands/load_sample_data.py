...
class Command(BaseCommand):
            ...
            for row in csv.DictReader(f):
                ...
                Property.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        "has_electricity_backup": bool_val(
                            row["has_electricity_backup"]
                        ),
    ...
    def load_property_photos(self):
        with open(f"{CSV_DIR}/property_photos.csv") as f:
            for row in csv.DictReader(f):
                ...
                PropertyPhoto.objects.get_or_create(
                    id=row["id"],
                    defaults={
                        "property": prop,
                        "image": row["image"],
        ...
        with open(f"{CSV_DIR}/room_facilities.csv") as f:
            for row in csv.DictReader(f):
                ...
                RoomFacility.objects.get_or_create(
                    room=room,
                    defaults={field: bool_val(row[field]) for field in bool_fields},
                )
    ...
    def load_room_photos(self):
        with open(f"{CSV_DIR}/room_photos.csv") as f:
            for row in csv.DictReader(f):
                room = Room.objects.get(id=row["room_id"])
                RoomPhoto.objects.get_or_create(
                    id=row["id"],
                    defaults={
                        "room": room,
                        "image": row["image"],
