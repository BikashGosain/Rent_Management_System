...
def admin_dashboard(request):
    ...
    context = {
        ...
        "open_complaints": Complaint.objects.filter(status="open").order_by(
            "-created_at"
        )[:5],
    }
...
def owner_dashboard(request):
    ...
    ) | Booking.objects.filter(room__property__owner=request.user, status="pending")
    recent_bookings = Booking.objects.filter(
        property__owner=request.user
    ) | Booking.objects.filter(room__property__owner=request.user)
    ...
    total_received = (
        Payment.objects.filter(owner=request.user, status="paid").aggregate(
            t=Sum("amount")
        )["t"]
        or 0
    )
    pending_payments = Payment.objects.filter(
        owner=request.user, status__in=["pending", "overdue"]
    ).count()
    ...
    activity = []
    for b in Booking.objects.filter(property__owner=request.user).order_by(
        "-created_at"
    )[:3]:
        activity.append(
            {
                "icon": "🏠",
                "text": f"New booking request for {b.get_target_name()}",
                "time": b.created_at,
                "link": f"/bookings/{b.pk}/",
            }
        )
    for p in Payment.objects.filter(owner=request.user, status="paid").order_by(
        "-paid_date"
    )[:3]:
        activity.append(
            {
                "icon": "💰",
                "text": f"Payment received: Rs. {p.amount}",
                "time": p.updated_at,
                "link": f"/payments/{p.pk}/",
            }
        )
    for c in Complaint.objects.filter(owner=request.user).order_by("-created_at")[:3]:
        activity.append(
            {
                "icon": "⚠️",
                "text": f"Complaint: {c.title}",
                "time": c.created_at,
...
def tenant_dashboard(request):
    ...
    for b in bookings[:3]:
        activity.append(
            {
                "icon": "🏠",
                "text": f"Booking {b.get_status_display()} — {b.get_target_name()}",
                "time": b.updated_at,
                "link": f"/bookings/{b.pk}/",
            }
        )
    for a in agreements[:3]:
        activity.append(
            {
                "icon": "📋",
                "text": f"Agreement {a.get_status_display()} — {a.get_target_name()}",
                "time": a.updated_at,
                "link": f"/agreements/{a.pk}/",
            }
        )
    for p in payments.order_by("-updated_at")[:3]:
        activity.append(
            {
                "icon": "💰",
                "text": f"Payment {p.get_status_display()} — Rs. {p.amount}",
                "time": p.updated_at,
                "link": f"/payments/{p.pk}/",
            }
        )
    ...
    context = {
        ...
        "total_paid": payments.filter(status="paid").aggregate(t=Sum("amount"))["t"]
        or 0,
