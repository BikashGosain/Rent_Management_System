...
class SearchAPI(APIView):
    ...
    def get(self, request):
        ...
        # What to return
        result_type = request.query_params.get('result_type', 'all')  # all | properties | rooms

        # ── Filter Properties (mirrors your search_view) ───────────────────
        properties = Property.objects.filter(
            status='available'
        ).select_related('owner').prefetch_related('photos', 'rooms')
        ...
        if rent_type:  properties = properties.filter(rent_type=rent_type)
        if furnishing: properties = properties.filter(furnishing=furnishing)
        if bedrooms:   properties = properties.filter(total_bedrooms__gte=bedrooms)
        if min_price:  properties = properties.filter(rent_price__gte=min_price)
        if max_price:  properties = properties.filter(rent_price__lte=max_price)
        if has_wifi:   properties = properties.filter(has_internet=True)
        if has_parking:properties = properties.filter(has_parking=True)
        ...
        if city:       rooms = rooms.filter(property__city__icontains=city)
        if location:
            rooms = (
                rooms.filter(property__address__icontains=location) |
                rooms.filter(property__landmark__icontains=location)
            )
        if prop_type:  rooms = rooms.filter(property__type=prop_type)
        if furnishing: rooms = rooms.filter(furnishing=furnishing)
        if bedrooms:   rooms = rooms.filter(bedrooms__gte=bedrooms)
        if min_price:  rooms = rooms.filter(rent_price__gte=min_price)
        if max_price:  rooms = rooms.filter(rent_price__lte=max_price)
        if has_wifi:   rooms = rooms.filter(facility__wifi=True)
        if has_ac:     rooms = rooms.filter(facility__ac=True)
        if has_parking:rooms = rooms.filter(facility__parking=True)
        if has_laundry:rooms = rooms.filter(facility__laundry=True)
        if has_lift:   rooms = rooms.filter(facility__lift=True)
        if has_cctv:   rooms = rooms.filter(facility__cctv=True)
        ...
        if result_type in ['all', 'properties']:
            ...
            whole_data = SearchPropertySerializer(whole_page, many=True, context=ctx).data

            # Room-type properties
            room_prop_page = paginator.paginate_queryset(room_properties, request)
            room_prop_data = SearchPropertySerializer(room_prop_page, many=True, context=ctx).data
