...
def search_view(request):
    properties = Property.objects.filter(status="available").prefetch_related(
        "photos", "rooms"
    )
    rooms = (
    ...
    if location:
        properties = properties.filter(address__icontains=location) | properties.filter(
    ...
    context = {
        ...
        "rooms": rooms,
        "total_results": whole_properties.count()
        + room_properties.count()
        + rooms.count(),
        "filters": {
            "city": city,
            "location": location,
