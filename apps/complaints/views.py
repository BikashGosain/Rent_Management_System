    ...
    if request.method == "POST":
        ...
        if form.is_valid() and agreement_pk:
            agreement = get_object_or_404(
                Agreement, pk=agreement_pk, tenant=request.user
            )
...
def owner_complaints(request):
    ...
    complaints = (
        Complaint.owner_objects.filter(owner=request.user)
        .prefetch_related("responses")
    ...
    return render(
        request,
        "complaints/owner_complaints.html",
        {
...
def admin_complaints(request):
    ...
    complaints = (
        ...
        .select_related("tenant", "owner", "property", "room")
        .prefetch_related("responses")
    )

    return render(
        request, "complaints/admin_complaints.html", {"complaints": complaints}
    )
...
def owner_submit_complaint(request, agreement_pk):
    ...
    agreement = get_object_or_404(
        Agreement, pk=agreement_pk, owner=request.user, status="active"
    )
    ...
    return render(
        request,
        "complaints/owner_complaint_form.html",
...
def delete_complaint(request, pk):
    ...
    if complaint.status not in allowed:
        ...
        return redirect(
            "complaints:owner_complaints" if is_owner else "complaints:my_complaints"
        )
    ...
    return render(
        request, "base_delete_confirm.html", {"object": complaint, "type": "Complaint"}
    )
