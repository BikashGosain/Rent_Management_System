...
from .serializers import (
    BookingSerializer, BookPropertySerializer,
    BookRoomSerializer, OwnerResponseSerializer,
)
...
class MyBookingsAPI(generics.ListAPIView):
    ...
    def get_queryset(self):
        qs = Booking.tenant_objects.filter(
...
class OwnerBookingsAPI(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        qs = Booking.owner_objects.filter(
            property__owner=user
        ).select_related('tenant', 'property', 'room', 'room__property') | \
        Booking.owner_objects.filter(
