...
def book_property(request, pk):
    ...
    if Booking.objects.filter(
        ...
        messages.warning(
            request, "You already have a pending booking for this property."
        )
        return redirect("properties:detail", pk=pk)
...
def book_room(request, property_pk, room_pk):
    ...
    room = get_object_or_404(
        Room, pk=room_pk, property__pk=property_pk, status="available"
    )
...
def my_bookings(request):
    ...
    bookings = (
        Booking.tenant_objects.filter(tenant=request.user)
...
def owner_bookings(request):
    ...
    ) | Booking.owner_objects.filter(room__property__owner=request.user).select_related(
        "tenant", "property", "room", "room__property"
    )
...
def booking_detail(request, pk):
    ...
    cancelled_by_owner = (
        booking.status == "cancelled" and booking.cancelled_by == "owner"
    )
    cancelled_by_tenant = (
        booking.status == "cancelled" and booking.cancelled_by == "tenant"
    )
...
def delete_booking(request, pk):
    ...
    if booking.status not in allowed:
        messages.error(
            request,
            f"Cannot delete a booking with status: {booking.get_status_display()}",
        )
        return redirect("bookings:my_bookings")
    ...
    return render(
        request, "base_delete_confirm.html", {"object": booking, "type": "Booking"}
    )
