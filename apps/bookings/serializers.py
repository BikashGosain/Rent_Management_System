...
class BookingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model  = Booking
        fields = [
            'id', 'status', 'move_in_date', 'move_out_date',
            'message', 'owner_note', 'cancelled_by',
            'created_at', 'updated_at',
            'tenant_name', 'tenant_phone',
            'target_name', 'target_type', 'rent_price', 'owner_name',
            'property', 'room',
        ]
...
class BookPropertySerializer(serializers.ModelSerializer):
    ...
    def validate_property(self, prop):
        ...
        if prop.rent_type != 'whole':
            raise serializers.ValidationError('This property rents individual rooms, not whole.')
        ...
        if Booking.objects.filter(tenant=request.user, property=prop, status='pending').exists():
            raise serializers.ValidationError('You already have a pending booking for this property.')
        return prop
...
class BookRoomSerializer(serializers.ModelSerializer):
    ...
    def validate_room(self, room):
        ...
        if Booking.objects.filter(tenant=request.user, room=room, status='pending').exists():
            raise serializers.ValidationError('You already have a pending booking for this room.')
        return room
