...
class Payment(SoftDeleteModel):
    ...
    agreement = models.ForeignKey(
        Agreement, on_delete=models.CASCADE, related_name="payments"
    )
    ...
    payment_type = models.CharField(
        max_length=20, choices=PAYMENT_TYPES, default="rent"
    )
    ...
    month = models.PositiveIntegerField(
        null=True, blank=True, help_text="Month number (1-12)"
    )
    year = models.PositiveIntegerField(
        null=True, blank=True, help_text="Year (e.g. 2026)"
    )

    # Extra info
    notes = models.TextField(blank=True)
    transaction_id = models.CharField(
        max_length=100, blank=True, help_text="Bank/online transaction ID"
    )
