...
def owner_payments(request):
    ...
    total_received = (
        payments.filter(status="paid").aggregate(
            total=__import__("django.db.models", fromlist=["Sum"]).Sum("amount")
        )["total"]
        or 0
    )
    total_pending = (
        payments.filter(status__in=["pending", "overdue"]).aggregate(
            total=__import__("django.db.models", fromlist=["Sum"]).Sum("amount")
        )["total"]
        or 0
    )
...
def create_payment(request, agreement_pk):
    ...
    return render(
        request, "payments/payment_form.html", {"form": form, "agreement": agreement}
    )
...
def auto_generate_payments(request, agreement_pk):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if created:
                messages.success(
                    request, f"{created} monthly payment(s) generated successfully!"
                )
            else:
    ...
    return render(
        request, "payments/auto_generate_confirm.html", {"agreement": agreement}
    )
...
def mark_paid(request, pk):
    ...
    if request.method == "POST":
        ...
        form = PaymentMarkPaidForm(
            instance=payment, initial={"paid_date": timezone.now().date()}
        )

    return render(
        request, "payments/mark_paid_form.html", {"form": form, "payment": payment}
    )
...
def delete_payment(request, pk):
    ...
    if payment.status not in allowed:
        ...
        return redirect(
            "payments:owner_payments" if is_owner else "payments:tenant_payments"
        )
    ...
    return render(
        request,
        "base_delete_confirm.html",
        {
            "object": payment,
            "type": "Payment",
            "cancel_url": "payments:owner_payments"
            if is_owner
            else "payments:tenant_payments",
        },
    )
