    ...
    def clean(self):
        ...
        if rental_type == "fixed":
            if not end_date:
                raise forms.ValidationError(
                    "End date is required for fixed term rental."
                )
        ...
        if rental_type == "short":
            if not cleaned.get("short_term_unit"):
                raise forms.ValidationError(
                    "Please select daily or weekly for short term rental."
                )
            if not cleaned.get("short_term_duration"):
                raise forms.ValidationError(
                    "Please enter duration for short term rental."
                )
...
class NoticeForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "notice_reason": forms.Textarea(
                attrs={"rows": 3, "placeholder": "Explain your reason..."}
            ),
        }
        labels = {
...
class NoticeResponseForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "notice_response": forms.Textarea(
                attrs={"rows": 3, "placeholder": "Your response to the notice..."}
            ),
        }
        labels = {
...
class ExtensionRequestForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "extension_reason": forms.Textarea(
                attrs={"rows": 3, "placeholder": "Why do you want to extend your stay?"}
            ),
...
class ExtensionResponseForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "extension_response": forms.Textarea(
                attrs={"rows": 3, "placeholder": "Your response..."}
            ),
        }
        labels = {
