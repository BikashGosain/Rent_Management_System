...
def submit_notice(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not notice.notice_vacate_date:
                notice.notice_vacate_date = timezone.now().date() + \
                    __import__('datetime').timedelta(days=agreement.notice_period_days)
        ...
        if is_tenant:
            ...
            initial['notice_vacate_date'] = timezone.now().date() + \
                __import__('datetime').timedelta(days=agreement.notice_period_days)
        else:
    ...
    return render(request, 'agreements/notice_form.html', {
        'form':      form,
        'agreement': agreement,
        'is_owner':  is_owner,
        'is_tenant': is_tenant,
...
def respond_notice(request, pk):
    ...
    return render(request, 'agreements/notice_response_form.html', {
...
def complete_vacate(request, pk):
    ...
    if request.method == 'POST':
        ...
        agreement.terminated_by     = request.user
        agreement.terminated_at     = timezone.now()
        agreement.termination_reason = f'Vacated as per notice. Vacate date: {agreement.notice_vacate_date}'
