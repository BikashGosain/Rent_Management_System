    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if ok:
                messages.success(
                    request,
                    f"OTP sent to {user.email} and {user.phone or 'your phone'}.",
                )
            else:
                messages.warning(
                    request, "Account created but OTP delivery failed. Use resend."
                )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if purpose == "register":
                ...
                messages.success(
                    request,
                    f"Welcome to RentMS, {user.first_name or user.username}! 🎉",
                )
                return redirect("core:home")
...
def resend_otp_view(request):
    ...
    if ok:
        messages.success(
            request,
            f"New OTP sent to {mask_email(user.email)} and {mask_phone(user.phone)}.",
        )
    else:
...
def social_complete_view(request):
    ...
    if request.method == "POST":
        ...
        if role in ["owner", "tenant"]:
            ...
            messages.success(
                request, f"Welcome to RentMS, {user.first_name or user.username}! 🎉"
            )
            return redirect("core:home")
        else:
