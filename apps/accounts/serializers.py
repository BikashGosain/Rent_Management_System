    ...
    def validate(self, data):
        ...
        if not otp:
            raise serializers.ValidationError(
                {"code": "No active OTP found. Please resend."}
            )
        if otp.is_expired():
            raise serializers.ValidationError({"code": "OTP expired. Please resend."})
        if otp.attempts >= 3:
            raise serializers.ValidationError(
                {"code": "Too many attempts. Please resend OTP."}
            )
        ...
        if otp.code != data["code"]:
            ...
            raise serializers.ValidationError(
                {"code": f"Wrong OTP. {remaining} attempt(s) left."}
            )
...
class ResendOTPSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        last_otp = (
            OTP.objects.filter(user=user, purpose=data["purpose"])
            .order_by("-created_at")
            .first()
        )
        ...
        if last_otp and not last_otp.can_resend():
            ...
            raise serializers.ValidationError(
                {"detail": f"Wait {wait} seconds before resending."}
            )
