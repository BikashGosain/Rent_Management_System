...
def write_review(request, agreement_pk, review_type):
    ...
    if agreement.status not in ['active', 'terminated', 'expired']:
        messages.error(request, 'You can only review after an agreement is active or completed.')
...
def reviews_received(request):
    ...
    return render(request, 'reviews/reviews_received.html', {
        'reviews':    reviews,
