...
def toggle_property_bookmark(request, pk):
    ...
    messages.success(request, 'Added to favorites!' if bookmarked else 'Removed from favorites.')
...
def toggle_compare(request, pk):
    ...
    if pk in compare_ids:
        ...
        if len(compare_ids) >= 3:
            if request.headers.get('x-requested-with') == 'XMLHttpRequest':
                return JsonResponse({'error': 'max', 'message': 'You can compare up to 3 properties at a time.'})
...
def compare_view(request):
    ...
    return render(request, 'bookmarks/compare.html', {
        'bookmarks':   bookmarks,
        'compare_ids': compare_ids,
    })
